/*
 ...
 [task_local]
 #泡泡大战
 1 0 * * * https://raw.githubusercontent.com/RW689/JD_Script/master/jd_ppdz.js, tag=泡泡大战, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
*/
    ...
    setTimeout( ()=>{
      ...
      $.get(url, async (err, resp, data) => {
        try {
          ...
          console.log(data)
          //await notify.sendNotify(`${$.name} - jxgc`, `京东账号${$.index} ${$.nickName}`+"电动车制造："+data)
         if (data.code === 0) {
            // console.log("赚京豆步数换京豆:2000步"+data.msg)
          //await notify.sendNotify(`${$.name} - 赚京豆步数换京豆`, `京东账号${$.index} ${$.nickName}`+"\n赚京豆步数换京豆:2000步\n"+data.msg)
            } else {
               //console.log("赚京豆步数换京豆:2000步"+data.msg)
               //await notify.sendNotify(`${$.name} - 赚京豆步数换京豆`, `京东账号${$.index} ${$.nickName}`+"\n赚京豆步数换京豆2000步\n步数不足或今日你已经兑换")
            }
        ...
        try {
         ...
         if (result.status === 0) {
          ...
          $.log(ts);
             //$.log("\n当前个人积分："+score+"\n当前个人排名："+pm)
            // $.log("\n开始时间："+kssj+"\n结束时间："+jssj)
          //await notify.sendNotify(`${$.name} - 东东泡泡大战`, `京东账号${$.index} ${$.nickName}`+`\n东东泡泡大战,今日任务已完成\n`+`\n当前个人积分：`+score+`\n当前个人排名：`+pm+`\n开始时间：`+kssj+`\n结束时间：`+jssj)
            } else {
               //console.log("赚京豆步数换京豆:2000步"+data.msg)
               //await notify.sendNotify(`${$.name} - 赚京豆步数换京豆`, `京东账号${$.index} ${$.nickName}`+"\n赚京豆步数换京豆2000步\n步数不足或今日你已经兑换")
            }
